/*
 * file:        mkfs-x6.c
 * description: create empty file system for cs5600/cs7600 file sys. assignment
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
int main(int argc, char **argv)
{
    ...
    int n_ino_blks = DIV_ROUND_UP(n_inos*sizeof(struct fs7600_inode),
                                  FS_BLOCK_SIZE);
    ...
    struct fs7600_inode *inodes = (void*)(disk + inode_base*FS_BLOCK_SIZE);

    int rootdir_base = inode_base + n_ino_blks;
    struct fs7600_dirent *de = (void*)(disk + rootdir_base*FS_BLOCK_SIZE);

    /* superblock */
    *sb = (struct fs7600_super){.magic = FS7600_MAGIC, .inode_map_sz = n_ino_map_blks,
