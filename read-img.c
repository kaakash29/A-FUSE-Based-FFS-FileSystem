...
int main(int argc, char **argv)
{
    ...
    struct fs7600_inode *inodes = (void*)block_map + sb->block_map_sz * FS_BLOCK_SIZE;
    
    struct entry { int dir; int inum;} inode_list[100];
    int head = 0, tail = 0;

    inode_list[head++] = (struct entry){.dir=1, .inum=1};
    while (head != tail) {
        struct entry e = inode_list[tail++];
        struct fs7600_inode *in = inodes + e.inum;
        if (!e.dir) {
            printf("file: inode %d\n"
                   "      uid/gid %d/%d\n"
                   "      mode %08o\n"
            ...
            struct fs7600_dirent *de = disk + in->direct[0] * FS_BLOCK_SIZE;
